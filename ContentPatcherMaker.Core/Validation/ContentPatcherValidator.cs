...
{
    private readonly List<ValidationError> _validationResults = [];
    ...
    private void ValidateChanges(List<Patch> changes)
    {
        ...
        {
            // Changes字段可以为空，如果ContentPack只包含ConfigSchema、CustomLocations或DynamicTokens
            return;
        }
    ...
    private void ValidateEditImagePatch(EditImagePatch patch, int index)
    {
        ...
        // 验证区域
        ValidateArea(patch.FromArea, $"EditImage补丁 {index} 的FromArea");
        ValidateArea(patch.ToArea, $"EditImage补丁 {index} 的ToArea");
    }
    ...
    private void ValidateEditMapPatch(EditMapPatch patch, int index)
    {
        ...
        // 验证区域
        ValidateArea(patch.FromArea, $"EditMap补丁 {index} 的FromArea");
        ValidateArea(patch.ToArea, $"EditMap补丁 {index} 的ToArea");
    ...
    /// 验证区域
    /// </summary>
    private void ValidateArea(Area? area, string fieldName)
    {
        if (area == null) return;
    ...
    private void ValidateMapTile(MapTile tile, int patchIndex)
    {
        ...
        {
            ValidateArea(tile.Position, $"补丁 {patchIndex} 的MapTile的Position");
        }
    }
